...
<div class="card">
    <div class="card-body">
        ...
        <table class="table">
            ...
            <tbody>
                ...
                ?>
                    ...
                    foreach ($statistik as $h): ?>
                        <tr>
                            <td><?= $i++; ?></td>
                            <td>
                                <?php if (!empty($h['icon'])) { ?>
                                    <img src="/assets/img/statistik/<?= $h['icon'] ?>" style="height: 50px;" class="me-3 rounded">
                                <?php } else { ?>
                            ...
                            <td class="text-center">
                                <a href="/ladmin/statistik/edit/<?= $h['id_statistik'] ?>" class="btn btn-success btn-sm"><i class="ri-pencil-fill"></i></a>
                                <a href="javascript:void(0)" onclick="hapus('<?= $h['id_statistik'] ?>')" class="btn btn-danger btn-sm"><i class="ri-close-line"></i></a>
                            </td>
                        </tr>
                <?php
                    endforeach;
        ...
        <?= $pager->links('statistik', 'lintang_pagination') ?>
    </div>
</div>
<?= $this->endSection(); ?>
    ...
    function hapus(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "<?= base_url(); ?>ladmin/statistik/delete/" + id,
                    type: "POST",
                    dataType: "JSON",
