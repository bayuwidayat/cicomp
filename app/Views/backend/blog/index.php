                        ...
                        <tr>
                            ...
                            <td>
                                <?php if (!empty($h['banner'])) { ?>
                                    <img src="/assets/img/blog/<?= $h['banner'] ?>" style="height: 50px;" class="me-3 rounded">
                                <?php } else { ?>
                            ...
                            <td class="text-center">
                                <a href="/ladmin/blog/edit/<?= $h['id_blog'] ?>" class="btn btn-success btn-sm"><i class="ri-pencil-fill"></i></a>
                                <a href="javascript:void(0)" onclick="hapus('<?= $h['id_blog'] ?>')" class="btn btn-danger btn-sm"><i class="ri-close-line"></i></a>
                            </td>
                        </tr>
                <?php
                    endforeach;
    ...
    function hapus(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "<?= base_url(); ?>ladmin/blog/delete/" + id,
                    type: "POST",
                    dataType: "JSON",
